/**
 * Athlete superpowers API - GET (list), POST (add preset or custom), DELETE (remove).
 */
...
export async function GET(
  ...
  try {
    const { rows } = await sql`
      SELECT s.id, s.athlete_id, s.preset_id, s.custom_text, s.detail, s.display_order, s.created_at, p.label AS preset_label
      FROM athlete_superpowers s
      LEFT JOIN superpower_presets p ON p.id = s.preset_id
    ...
    console.error("GET /api/athletes/[id]/superpowers:", err);
    return NextResponse.json(
      { error: "Failed to fetch superpowers" },
      { status: 500 }
    );
  }
}
...
export async function POST(
  ...
  try {
    ...
    const { rows } = await sql`
      INSERT INTO athlete_superpowers (athlete_id, preset_id, custom_text, detail, display_order)
    ...
    if (msg.includes("superpower_preset_or_custom") || msg.includes("check")) {
      return NextResponse.json(
        { error: "Provide exactly one of preset_id or custom_text" },
        { status: 400 }
      );
    }
    console.error("POST /api/athletes/[id]/superpowers:", err);
    return NextResponse.json(
      { error: "Failed to add superpower" },
      { status: 500 }
    );
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    const { rows } = await sql`
      DELETE FROM athlete_superpowers
    ...
    console.error("DELETE /api/athletes/[id]/superpowers:", err);
    return NextResponse.json(
      { error: "Failed to remove superpower" },
      { status: 500 }
    );
  }
}
