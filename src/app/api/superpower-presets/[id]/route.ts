/**
 * Single superpower preset API - GET, PUT (auth), DELETE (auth).
 */
...
export async function GET(
  ...
  try {
    ...
    console.error("GET /api/superpower-presets/[id]:", err);
...
export async function PUT(
  ...
  try {
    ...
    if (label !== undefined) {
      const { rows } = await sql`
        UPDATE superpower_presets SET label = ${label.trim()} WHERE id = ${id}
        RETURNING id, label, display_order, created_at
      `;
      if (rows.length === 0) {
        return NextResponse.json({ error: "Preset not found" }, { status: 404 });
    ...
    if (display_order !== undefined) {
      const order = Number(display_order);
      const { rows } = await sql`
        UPDATE superpower_presets SET display_order = ${order} WHERE id = ${id}
        RETURNING id, label, display_order, created_at
      `;
      if (rows.length === 0) {
        return NextResponse.json({ error: "Preset not found" }, { status: 404 });
    ...
    console.error("PUT /api/superpower-presets/[id]:", err);
    return NextResponse.json(
...
export async function DELETE(
  ...
  try {
    const { rows } = await sql`
      DELETE FROM superpower_presets WHERE id = ${id}
      RETURNING id
    `;
    if (rows.length === 0) {
      return NextResponse.json({ error: "Preset not found" }, { status: 404 });
    ...
    console.error("DELETE /api/superpower-presets/[id]:", err);
    return NextResponse.json(
