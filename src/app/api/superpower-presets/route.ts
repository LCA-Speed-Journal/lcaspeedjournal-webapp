/**
 * Superpower presets API - GET (public list), POST (auth required).
 */
...
export async function GET() {
  try {
    ...
    console.error("GET /api/superpower-presets:", err);
    return NextResponse.json(
      { error: "Failed to fetch superpower presets" },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { rows } = await sql`
      INSERT INTO superpower_presets (label, display_order)
    ...
    console.error("POST /api/superpower-presets:", err);
    return NextResponse.json(
      { error: "Failed to create superpower preset" },
      { status: 500 }
    );
  }
}
