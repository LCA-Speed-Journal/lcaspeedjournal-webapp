...
- [ ] Initialize Next.js project (TypeScript, App Router, ESLint, `src/` optional)
- [ ] Add Vercel Postgres; run migrations for `athletes`, `sessions`, `entries`, `athlete_notes`
- [ ] Configure NextAuth (credentials / coach PIN); protect write APIs
- [ ] Port metric registry (`config/metrics.json`) into repo (e.g. `lib/metrics.json` or `data/metrics.json`)
- [ ] Local dev: `npm run dev`, one table + one API read/write smoke test
