 ...
 *
 * Run: node scripts/seed-historical.cjs
 * Or:  SEED_DATA_DIR="C:\path\to\LCA-Speed-Journal\data\historical" node scripts/seed-historical.cjs
 *
...
async function main() {
  ...
  const athleteKey = (first, last, grad) => `${first}|${last}|${grad ?? ""}`;
  const athleteMap = new Map();
  const sessionKey = (date, phase, week) => `${date}|${phase}|${week}`;
  ...
  for (const [key, a] of athleteMap) {
    ...
    if (rows.length > 0) {
      ...
      const ins = await sql`
        ...
        RETURNING id
      `;
      athleteIdByKey.set(key, ins.rows[0].id);
    }
  }
  ...
  for (const [key, s] of sessionMap) {
    ...
    if (rows.length > 0) {
      ...
      const ins = await sql`
        ...
        RETURNING id
      `;
      sessionIdByKey.set(key, ins.rows[0].id);
    }
  }
